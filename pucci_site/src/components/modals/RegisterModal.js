...
const RegisterModal = (props) => {
  ...
  const validate = (values) => {
    ...
    if (!nameRegex.test(values.firstName)) {
      setFormErrors({
        firstname: "Can only contain upper and lowercase letters.",
      });
      return false;
    }
    if (!nameRegex.test(values.lastName)) {
      setFormErrors({
        lastname: "Can only contain upper and lowercase letters.",
      });
      return false;
    }
    // phone
        ...
        <div className="reg-box-content">
          ...
          <div className="error-container">
            ...
            {formErrors.password2 !== "" ? (
              <div className="error-message">{formErrors.password2}</div>
            ) : (
              ""
            )}
          </div>
